...
import torch
from torch.distributions import Distribution, Bernoulli, Normal, Beta
...
from probabll.dgm.likelihood import (
    FullyFactorizedLikelihood,
...
class InferenceModel(torch.nn.Module):
    def __init__(
        self, x_size, z_size, y_size, conditional_z: ConditionalLayer
    ):
    ...
    def rsample(self, q_z):
        ...
        # else:
        z = q_z.rsample()
        _, djac = None, torch.zeros_like(z)
    ...
    def forward(self, x, y, inf_model, num_samples):
        ...
        for s in range(num_samples):
            ...
            ll.append(
                # [1, B]
                (
                    log_conditional_x + p_z.log_prob(z).sum(-1) - log_q_z
                ).unsqueeze(0)
            )
            # [B]
...
def config(**kwargs):
    ...
    parser.add_argument(
        "--logdir", type=str, default=None, help="Tensorboard logdir"
    )
...
class Experiment:
    ...
    * load a trained model
        ...
        for epoch in range(args.epochs):
            ...
            for i, (x_mb, y_mb) in enumerate(iterator):
                ...
                iterator.set_postfix(
                    OrderedDict(
                        (k, "{:4.2f}".format(v)) for k, v in loss_dict.items()
                    ),
                    refresh=False,
                )
                step += 1
            ...
            if stop:
                print(
                    "Early stopping at epoch {:3}/{}".format(
                        epoch + 1, args.epochs
                    )
                )
                break
            ...
            print(
                "Epoch {:3}/{} -- LL {:.2f} -- ".format(
                    epoch + 1, args.epochs, val_ll
                )
                + ", ".join(
                    [
                        "{}: {:4.2f}".format(k, v)
    ...
    def log_likelihood(self, batcher: Batcher, num_samples=10):
        ...
        (note this will not update the learning rate scheduler"""
        args, gen_model, inf_model = (
            self.args,
            self.models["gen"],
            self.models["inf"],
        )
    ...
    def validate(self):
        ll, return_dict = self.log_likelihood(
            self.get_batcher(self.valid_loader), self.args.ll_samples,
        )
    ...
    def test(self):
        ...
        (note this will not update the learning rate scheduler)"""
        ll, return_dict = self.log_likelihood(
            self.get_batcher(self.test_loader), self.args.ll_samples,
        )
        return ll, return_dict
