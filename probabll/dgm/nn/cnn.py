...
class MaskedLinear(nn.Module):
    ...
    def __init__(
        self, in_features, out_features, diagonal_zeros=False, bias=True
    ):
...
class MaskedConv2d(nn.Module):
    ...
    def build_mask(self):
        ...
        assert n_out % n_in == 0 or n_in % n_out == 0, "%d - %d" % (
            n_in,
            n_out,
        )
        ...
        mask = np.ones(
            (n_out, n_in, self.size_kernel[0], self.size_kernel[1]),
            dtype=np.float32,
        )
        mask[:, :, :l, :] = 0
        mask[:, :, l, :m] = 0
    ...
    def forward(self, x):
        output = F.conv2d(
            x, self.mask * self.weight, bias=self.bias, padding=(1, 1)
        )
        return output
