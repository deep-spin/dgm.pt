...
class FFConditioner(Conditioner):
    ...
    def __init__(
        ...
        net = [
            ...
            for h0, h1 in zip(
                [input_size - context_size] + hidden_sizes, hidden_sizes
            )
        ]
        ...
        if context_size > 0:
            ctxt_net = [
                torch.nn.Linear(context_size, units) for units in hidden_sizes
            ]
...
class MADEConditioner(Conditioner):
    ...
    def __init__(
        ...
        self._made = MADE(
            nin=input_size
            - context_size,  # inputs we are autoregressive about
            nout=output_size,
    ...
    def forward(self, inputs, num_samples=1, resample_mask=False, **kwargs):
        ...
        if self.context_size > 0:
            inputs, context = torch.split(
                inputs,
                [self.input_size - self.context_size, self.context_size],
                -1,
            )
        else:
            context = None
