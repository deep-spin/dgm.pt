"""
Helper code based on torch.distributions.register_kl
    to register a parameterization of
    torch.distributions.Distribution objects.

Prior parameterization differs from conditional parameterization
in that priors are specified via a list of floats.
...
def register_prior_parameterization(type_p):
    """
    Decorator to register a function that
    parameterizes a distribution of type_p.
    Usage::
    ...
    if not isinstance(type_p, type) and issubclass(type_p, Distribution):
        raise TypeError(
            "Expected type_p to be a Distribution subclass but got {}".format(
                type_p
            )
        )
...
def register_conditional_parameterization(type_p):
    ...
    that parameterizes a distribution of type_p.
    Usage::
    ...
    def decorator(fun):
        ...
        _CONDITIONAL_MEMOIZE.clear()  # reset; lookup order may have changed
        return fun
...
def parameterize_prior(
    ...
    Raises:
        NotImplementedError: If the distribution
            types have not been registered via
...
def parameterize_conditional(type_p, inputs, event_size) -> Distribution:
    ...
    Raises:
        NotImplementedError: If the distribution types
            have not been registered via
            :meth:`register_parameterization`.
    """
    try:
    ...
    if fun is NotImplemented:
        raise NotImplementedError(
            "I cannot find a parameterization for a conditional of type %s"
            % type_p
        )
...
def make_gaussian(inputs, event_size):
    assert (
        inputs.size(-1) == 2 * event_size
    ), "Expected [...,%d] got [...,%d]" % (2 * event_size, inputs.size(-1))
    params = torch.split(inputs, event_size, -1)
    return Normal(loc=params[0], scale=F.softplus(params[1]))
